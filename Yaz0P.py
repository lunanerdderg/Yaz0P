...
if (len(args) < 1 or help):
    print("""Yaz0P file [optional:output_directory] [--options]
...
try: # Test if file exists, and read binary from it
    ...
    print("File '" + inputPath + "' does not exist.")
    if waitBeforeExit:
        input("Press ENTER to exit program.")
    sys.exit()
...
if str(inputBinary)[2:6] != "Yaz0": # Test if file is compressed with Yaz0
    print("File '" + inputPath + "' is not compressed with Yaz0.")
    if waitBeforeExit:
        input("Press ENTER to exit program.")
    sys.exit()
...
try:
    ...
    print("Output directory does not exist, or error in the writing process.")
    if waitBeforeExit:
        input("Press ENTER to exit program.")
    sys.exit()
